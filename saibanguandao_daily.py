...
SELECT COUNT(imei)
FROM t_usmguserloginonline
WHERE  mcc = 310 AND mnc IN (140,370,470)
"""
...
def realtimeMonitor(startdate, non,new):
    nowdate = datetime.datetime.now().strftime("%d")
    if nowdate != startdate:
        partnerMonitor(non,new)
        realtimeMonitor(nowdate,0,0)
    else:
        today,non_new,new_new = initiate()
        realtimeMonitor(startdate,max(non,non_new),max(new,new_new))

if __name__ == "__main__":
    startdate, non,new = initiate()
    realtimeMonitor(startdate, non,new)
