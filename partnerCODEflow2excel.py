...
def publishQuery(period):
    yesterQuery = """
    SELECT * FROM
    t_flowsummary_partnercode_adv
    WHERE DATE(epochTime) = DATE_SUB(DATE(NOW()),INTERVAL 1 DAY)
    """
    weeklyQuery = """
    SELECT WEEK(NOW()) AS weeknum,iso2,partnercode,SUM(reserveup),SUM(reservedown),SUM(freeup),SUM(freedown),SUM(reservecnt),SUM(freecnt) FROM
    t_flowsummary_partnercode_adv
    WHERE DATE(epochTime) BETWEEN DATE_SUB(DATE(NOW()),INTERVAL 7 DAY) AND DATE_SUB(DATE(NOW()),INTERVAL 1 DAY)
    GROUP BY iso2,partnercode
    """
    rList = list()
    ...
    dateStr = str(y)
    filename = period + "_"+dateStr+"_partnerCODE.xlsx"
