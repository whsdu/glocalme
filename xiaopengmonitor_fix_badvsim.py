...
def monitorBestVsim(curFlowList):
    ...
    for row in curFlowList:
        ...
        tmpkey = str(imei)+str(sessionid)
        rcoTime = sessionTime.get(tmpkey,curTime)
        rat = imsiDetail[0]
        imsiISO2 = imsiDetail[1]

        if (rat == 2 or rat == 3) and iso2 == imsiISO2:
            col.update({'imei':imei,'sessionid':sessionid},
                               {'$set':{'imei':imei,'recordtime':rcoTime,'iso2':iso2,'belonging':'non','errtype':'badvsim','vip':1}},
                               upsert = True)
            sessionTime[tmpkey] = rcoTime
...
def queryandinsert():
    ...
    queryNonCurrent="""
    SELECT imei,vsimimsi AS imsi,sessionid,flowsize,visitcountry,sessionid FROM
    t_chgflowlog001
